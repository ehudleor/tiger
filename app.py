...
if app_mode == 'Segment an Image':
    ...
    leaf_count = np.sum((np.array(img) >0)&(np.array(img) <200))
    bg_count = np.sum((np.array(img) ==0)|(np.array(img) ==200))
    # Load Aruco detector
    ...
    st.subheader(f'the area of the laef:{Area:.3f} cm\N{SUPERSCRIPT TWO}')
